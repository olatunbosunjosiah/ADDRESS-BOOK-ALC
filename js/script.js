    ...
    ${$('#contact-list').innerHTML}
            <li  id="user-${user.id}">
                <a  data-toggle="collapse" 
                  ...
                  aria-expanded="false" 
                  aria-controls="user${user.id}Details" onclick="myFunction()" class="dropbtn"  id="user${user.id}Name" > ${user.firstname} ${user.lastname} </a>
            ...
            <div class="details collapse " id="user${user.id}Details">
              ...
              <div id="myDropdown${id}" class="dropdown-content">
                ...
                <a class="btn btn-primary" data-toggle="modal" dat  a-target="#myModal${user.id}" data-user-id="${user.id}" >Edit</a>           
                <!-- Modal -->
